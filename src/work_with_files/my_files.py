...
# 3.2. Напишите функцию, которая получает на вход два случайных числа от 1 до 10 и путь к файлам, по этим числам открывает 
# два соответствующих файла из задания выше, и возвращает сумму шести чисел (содержимое обоих файлов).
# Если содержимое любого файла неполно или испорчено, возвращайте информацию об ошибке (подумайте, как лучше организовать такую функцию).
...
# Барсик 5.0 75
# задаёт объект типа Cat (имя Барсик, масса 5.0, частота мурлыканья 75 Гц).
# Автоматически создавайте на основе такого набора строк в файле список объектов в вашей программе.
# Обрабатывайте случаи, когда некоторые строки будут содержать некорректные значения.
# Для разбиения строки, например, на список строк можете использовать стандартную функцию split() (погуглите), которая разбивает исходную строку по заданной строке-разделителю (например, пробелу " ").
...
print(res1)
res_cats = makeCats()
for mass_cat in res_cats:
    print(mass_cat.getCatName())    